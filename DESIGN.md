...
- **Structure**: bottom sheet with a header (title + "Kelola kategori" gear), search field, optional "no category" row, and the 3-level category tree (parents with indented, collapsible children). The picker is **selection-only**: it neither reorders nor creates in place.
